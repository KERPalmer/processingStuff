...
class Soldier {
  ...
  float facing = 0;
  float health = 100; 
  float speed = 2; // how fast they move
  color teamColor;
  color displayColor;
  ...
  Soldier(int id, float startingX, float startingY, color teamColor_, float size_) {
    pos= new Position(startingX, startingY);
    start = new Position(startingX, startingY);
    teamColor=teamColor_;
    size=size_;
    attackRange=size*3 + random(-size*0.1, size*0.1);
  }
  void display() {
    pushMatrix();
    ...
    for (Soldier soldier : alive) {
      ...
      if (soldier != this) {
        ...
        if (abs(pos.x-soldier.pos.x)<soldier.size*2 & abs(pos.y-soldier.pos.y)<soldier.size*2) {
          pos.x+=(pos.x-soldier.pos.x)/2.5;
          pos.y+=(pos.y-soldier.pos.y)/2.5;
        }
      }
    }
  }
  ...
  void attack(Soldier enemy) {
    ...
    wait=int(random(0, 60));
  }
  void RecieveDamage(float damage) {
    displayColor=hitColor;
    health-=damage;
    wait=int(random(0, 20));
  }
  void RunProcess(ArrayList<Soldier> alive, ArrayList<Soldier> oppositeTeamAlive, ArrayList<Soldier> teamAlive){
    if (oppositeTeamAlive.size()==0) {
      ...
      if (wait == 0) {
        setClosestEnemy(oppositeTeamAlive);
        boolean inAttackRange=checkAttackRange();
        if (inAttackRange) {
          attack(closestEnemy);
          display();
        } else {
          MoveTo(closestEnemy.pos.x,closestEnemy.pos.y);
          PreventOverlap(alive);
          PreventOverlap(alive);
          display();
        }
      } else {
...
ArrayList<Soldier> generateAlive(Soldier[] aTeam, Soldier[] bTeam) {
  ArrayList<Soldier> alive = new ArrayList();
...
ArrayList<Soldier> generateTeamAlive(Soldier[] team) {
  ArrayList<Soldier> alive = new ArrayList();
...
void runTurn(ArrayList<Soldier> team, ArrayList<Soldier> alive, ArrayList<Soldier> oppositeTeamAlive) {
  for (Soldier soldier : team) { 
    if (oppositeTeamAlive.size()==0) {
